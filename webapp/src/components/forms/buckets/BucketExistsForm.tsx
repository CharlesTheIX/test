...
const BucketExistsForm: React.FC = () => {
  ...
  const handleFormSubmission = async (): Promise<void> => {
    ...
    const request_data: Partial<Bucket> = { name };
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/buckets/exists`, {
        method: "POST",
  ...
  const validateRequest = (data: Partial<Bucket>): SimpleError => {
    ...
    Object.keys(data).map((key: string) => {
      switch (key) {
        case "name":
          invalid = getInputError("bucket_name", data[key], true);
          if (invalid.error) {
            inputs_invalid.name = invalid.error;
            message += `- Bucket name: ${invalid.message}\n`;
          }
          break;
      }
    });
  ...
  return (
    ...
    >
      <div className="content-container">
        <div className="inputs">
          <div className="w-full">
            <TextInput name="name" required={true} label="Bucket name" error={!!inputErrors.name} />
          </div>
        </div>
      ...
      {complete && <CompletionContainer title={`Bucket does${!exists ? " not " : " "}Exists`} />}
    </form>
  );
};

export default BucketExistsForm;
