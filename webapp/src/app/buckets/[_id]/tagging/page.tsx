...
import BucketsTaggingTable from "@/components/tables/buckets/BucketsTaggingTable";
...
export const generateMetadata = async ({ params }: { params: Promise<Params> }): Promise<Metadata> => {
  ...
  try {
    ...
    return {
      title: `${res.data._id} | Tagging | ${site_name}`,
      ...
      <main>
        <section>
          <div className="flex flex-row gap-2 items-center justify-between">
            <h1>{data.name}: Tagging</h1>

            <div className="flex flex-row gap-2 items-center">
              <PermissionsWrapper permissions={[9]}>
                <Link href={`/buckets/${data._id}/tagging/add`} className="hyve-button">
                  Add
                </Link>
              </PermissionsWrapper>
        ...
        <section>
          <BucketsTaggingTable bucket_id={_id} />
        </section>
      </main>
    );
  } catch (err: any) {
