...
import BucketTaggingForm from "@/components/forms/buckets/BucketTaggingForm";
...
export const generateMetadata = async ({ params }: { params: Promise<Params> }): Promise<Metadata> => {
  ...
  try {
    ...
    return {
      title: `${res.data._id} | Tagging | Add | ${site_name}`,
      ...
      <main>
        <section>
          <div className="flex flex-row gap-2 items-center justify-between">
            <h1>{data.name}: Edit Tagging</h1>

            <div className="flex flex-row gap-2 items-center">
              <Link href={`/buckets/${data._id}/tagging`} className="hyve-button cancel">
                Back
              </Link>
            </div>
          </div>
        ...
        <section>
          <BucketTaggingForm bucket_id={_id} />
        </section>
      </main>
    );
  } catch (err: any) {
