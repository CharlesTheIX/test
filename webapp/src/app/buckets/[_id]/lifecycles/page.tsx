...
import BucketsLifecyclesTable from "@/components/tables/buckets/BucketsLifecyclesTable";
...
export const generateMetadata = async ({ params }: { params: Promise<Params> }): Promise<Metadata> => {
  ...
  try {
    ...
    return {
      title: `${res.data._id} | Lifecycles | ${site_name}`,
      ...
      <main>
        <section>
          <div className="flex flex-row gap-2 items-center justify-between">
            <h1>{data.name}: Lifecycles</h1>

            <div className="flex flex-row gap-2 items-center">
              <PermissionsWrapper permissions={[9]}>
                <Link href={`/buckets/${_id}/lifecycles/add`} className={`hyve-button`}>
                  Add
                </Link>
              </PermissionsWrapper>
        ...
        <section>
          <BucketsLifecyclesTable bucket_id={_id} />
        </section>
      </main>
    );
  } catch (err: any) {
